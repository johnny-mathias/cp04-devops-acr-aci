...
aciNameMysql="rm563409-mysql-movies"
...
az container create \
  ...
  --environment-variables \
    SPRING_DATASOURCE_URL=$(az keyvault secret show --name spring-datasource-url --vault-name $keyVaultName --query value -o tsv | sed "s/rm563409-mysql-movies/$mysqlURL/") \
